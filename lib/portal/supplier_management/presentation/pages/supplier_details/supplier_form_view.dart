...
import 'package:medglobal_admin_portal/portal/supplier_management/presentation/pages/supplier_details/widgets/general_information.dart';
...
class SupplierFormView extends StatefulWidget {
  ...
  const SupplierFormView(this.dialogContext, {super.key, this.supplier});
...
class _SupplierFormViewState extends State<SupplierFormView> {
  late SupplierBloc _supplierCubit;
  ...
  void initState() {
    ...
    _supplierCubit = widget.dialogContext.read<SupplierBloc>();
  ...
  @override
  Widget build(BuildContext context) => BlocListener<SupplierBloc, SupplierState>(
        listener: (context, state) {
          final isCloseDialog = state.maybeWhen(
            success: (_) => true,
            failure: (_) => true,
            orElse: () => false,
          );
  ...
  void onDeleteSupplier() => _supplierCubit.add(SupplierEvent.deleteSupplier(_supplier!.id!, _supplier!.name));

  void onSaveSupplier() => isSupplierNew
      ? _supplierCubit.add(SupplierEvent.createSupplier(newSupplier))
      : _supplierCubit.add(SupplierEvent.updateSupplier(
          _supplier!.copyWith(
            name: _supplierNameController.text,
            mainContactName: _mainContactNameController.text,
            email: _emailController.text,
