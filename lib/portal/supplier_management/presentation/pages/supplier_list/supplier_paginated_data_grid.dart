...
class _SupplierPaginatedDataGridState extends State<SupplierPaginatedDataGrid> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<SupplierListCubit, SupplierListState>(listenWhen: (previous, current) {
      if (previous is SupplierListLoading && current is SupplierListLoaded) {
      ...
      if (state is SupplierListLoaded) {
        ...
        _supplierDataSource = SupplierDataSource(suppliers, _rowsPerPage);
      }
      ...
      return DataGridLoading(
        columns: DataGridUtil.getColumns(DataGridColumn.SUPPLIERS),
        source: _supplierDataSource = SupplierDataSource([], _rowsPerPage),
      );
    });
  }
}
...
class SupplierDataSource extends DataGridSource {
  SupplierDataSource(List<Supplier> suppliers, int rowsPerPage) {
    _rowsPerPage = rowsPerPage;
    _suppliers = suppliers;
    _paginatedSuppliers = _suppliers.getRange(0, suppliers.length).toList(growable: false);
    buildDataGridRows(_suppliers);
  }
  ...
  List<Supplier> _paginatedSuppliers = [];
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    ...
    if (startIndex < _suppliers.length && endIndex <= _suppliers.length) {
      _paginatedSuppliers = _suppliers.getRange(startIndex, endIndex).toList(growable: false);
      buildDataGridRows(_paginatedSuppliers);
      notifyListeners();
    } else {
      _paginatedSuppliers = [];
    }
  ...
  void buildDataGridRows(List<Supplier> suppliers) =>
      dataGridRows = suppliers.map((supplier) => supplier.toDataGridRow()).toList();
