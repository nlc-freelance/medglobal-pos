...
class OrderCubit extends HydratedCubit<OrderState> {
  ...
  void computeTotal() {
    ...
    double total = 0;
    if (order.subtotal != null) total = state.order.subtotal! - (order.totalDiscountFromAllItems);
...
double calculateSubtotal(List<OrderItem> items) =>
    roundToTwoDecimalPlaces(items.fold(0.0, (total, item) => total + item.totalOriginalPricePerItem));
